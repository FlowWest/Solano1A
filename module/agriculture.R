  ...
  tagList(
    fluidRow(
      column(width = 6,
             ...
             tags$p('Other crops types for which agricultural applied water demand was calculated, but are not presented in the table due to smaller acreages are: sod / grass seed, asparagus, oranges, olives, onion, truck crops, potatoes, dry beans, lettuce, and “other crops”—as categorized by the USDA.'),
             tags$p('Crop types with a “+” include the following crops per type.', tags$br(),
                    tags$b('Alfalfa+: '), 'Alfalfa, Rice, Vetch', tags$br(),
                    tags$b('Corn+: '), 'Corn, Sorghum, Sweet Corn, Double Crop Oats/Corn', tags$br(),
                    tags$b('Sunflower+: '), 'Sunflower, Cotton')),
      column(width = 6,
             tabsetPanel(
               tabPanel('Area',
